...
pub fn parse_args() -> Result<RingSettings, &'static str> {
            ...
            .arg(Arg::new("list") // Specify the input list file containing the ring's websites
                ...
                .help("Sets the input list file to use")) // Help message
			.arg(Arg::new("verbose") // Enable verbose logging mode (print more console messages)
                .short('v') // Short flag 
                .long("verbose") // Long flag 
                .action(ArgAction::SetTrue) // Makes it a flag without a value
                .help("Enables verbose logging")) 
            .arg(Arg::new("skip-verification") // Skip some error-checking when building webring 
            ...
            let skip_verify = *matches.get_one::<bool>("skip-verification").unwrap_or(&false); 

            let file_path_option = matches.get_one::<String>("list").cloned();

            if verbose
            {
                println!("Using website list: {}", file_path_option.clone().unwrap());
            }
